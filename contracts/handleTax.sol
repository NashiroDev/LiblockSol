...
contract TokenX is ERC20, ERC20Burnable, ERC20Permit, ERC20Votes {
    ...
    address private devWallet = payable(0x05525CdE529C5212F1eaB7f033146C8CC103cd5D);
    ...
    function calculateFeePercentage(uint amount)
        private
        pure
        returns (uint32)
    {
    ...
    function setFeeRecipientsAndShares(address _devWallet, uint16 _devWalletShares, address _distributionContract, uint16 _distributionContractShares, uint16 _liblockFondationWalletShares, uint16 _zeroAdressShares) external onlyAdmin {
        require(_devWalletShares+_distributionContractShares+_liblockFondationWalletShares+_zeroAdressShares == 1000, "Cummulated shares not equal to 1000");
